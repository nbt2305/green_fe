        ...
        public IActionResult Index(string datetime)
        {
            try
            {
                if (HttpContext.Session.GetInt32("RoleId") != null && (HttpContext.Session.GetInt32("RoleId").Value == 1 || HttpContext.Session.GetInt32("RoleId").Value == 2))
                {
                    ...
                    ProfitVM profitVM = GetDataFromApi<ProfitVM>($"https://db.chunchun.io.vn/api/DashBoard/GetProfit/{datetime}");
                    List<Account> userdata = GetDataFromApi<List<Account>>("https://db.chunchun.io.vn/api/DashBoard/GetListCustomer\r\n");
                    List<EventVM> events = new List<EventVM>();

                    // Fetch events from the API
                    var allEvents = GetDataFromApi<List<EventVM>>("https://db.chunchun.io.vn/api/Event/GetAllEvents");
